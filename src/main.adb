...
begin
  ...
  loop
    ...
    Put_line("<<->> Posrednik "&integer'image(idw)&" wybral bufor do pobrania ["&integer'image(z)&"]");
        ...
        if Pusty(Buff) then
          Put_line("<<->> Posrednik "&integer'image(idw)&" zawieszony na semaforze bufora ["&integer'image(z)&"]");
          SemE.PB;
        else
          Sem.PB;
          pobierz(Buff,x);
          Put_line("<<->> Posrednik "&integer'image(idw)&" pobral do bufora liczbe "&Integer'image(x)&", odwiesza semafor w buforze ["&integer'image(z)&"]");
          Sem.VB;
          SemF.VB;
        end if;
       Time:=Random(G) * 8.0;
      ...
      if pelny(Buff) then
              ...
              Sem.PB;
              wstaw(Buff,x);
              Put_line("<<-->> Posrednik "&integer'image(idw)&" wstawil do bufora liczbe "&Integer'image(x)&", odwiesza semafor w buforze ["&integer'image(z)&"]");
              Sem.VB;
              SemE.VB;
            end if;
    Reset(G);
